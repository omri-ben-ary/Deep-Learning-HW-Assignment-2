...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        #using the known derivative of the sigmoid function
        dx = out * (1- out) * dout
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        softmax_x = torch.log(torch.sum(torch.exp(x),dim=1,keepdim = True)) - x 
        correct_softmax_x = softmax_x[range(N),y]
        
        loss = torch.sum(correct_softmax_x)/N
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            drop_tensor = (torch.empty(x.size()).uniform_(0, 1) >= self.p) / (1 - self.p)
            out = torch.mul(x, drop_tensor)
            self.grad_cache['dropout'] = drop_tensor
        # ========================
    ...
    def backward(self, dout):
        ...
        dx = dout
        if self.training_mode:
            dx = torch.mul(dx, self.grad_cache['dropout'])
        # ========================
