...
Firstly we notice that for both K=32 and K=64 we got similar results, so our discussion is relavent to both. Secondly, it is very clear that L=16 did not learn at all, and since the next quesion discusses exactly this issue, for this discussion we will disregard the L=64 graphs. When we analyze the train accuracy it is clear that the deeper the network the better the accuracy, but not too deep as the L=16 is too deep. This makes senese because deeper networks increase model class capcity allowing for more complex data patterns to be learned by the model and allow more abstract high level features that improve the model's performence. As for the test accuracy results, we see that all models did quite the same with no major diffrences. This tells us, mainly for the deeper networks, that the models did not generalize well as we should expect to see better results on the deeper networks like we saw in train accuracy. To improve generalization we can use regularization methods more effectivly like dropout or weight decay. Putting all of these together, the model with L=8 produces the best results.

2.

In both cases (K=32 and K=64) we observe that the L=16 did not learn at all and was not trainable. This happens for multiple reasons. Firstly, in very deep networks we can come across vanishing/exploding gradients during training which effectively stops the learning. When gradients are too small, we stay in the same place in the hyperparameter space because the step size is almost zero due to gradient size. When gradients are too big, we take too big of a step which could cause us to move up along the lost function instead of downwards. Another reason that could explain why we were not able to train the network is the fact that the number of hyperparameters is much bigger in deeper networks which means that optimization process on a much bigger hyperparameter space (dimension wise) is much harder and extremly slower. To solve these problems we can take a few measures. As for exploding gradients we can use gradient clipping which limits the gradients size. For vanishing gradients we can use different activation functions like leaky ReLU that has non zero gradients. Also, we can use residual blocks that act as shortcuts in the network that allow us to effectivly skip layers and decrease this phenomenon. As for the harder optimzation problem we can use more sophisticated optimization algorithms like Adam or RMSprop which may do better. Also allowing for a longer optimzation time might help by tuning the early stopping parameter.
"""

part5_q2 = r"""
**Your answer:**
We notice across all different layer depth we tested that K=128 is the best result in terms of test accuracy acurracy. We can also see that K=64 did better than K=32. We can deduce that the larger filter sizes generate better results. This makes sense as the filter size corresponds to the models ability to capture more global features of the image instead of very local features. This is due to the larger receptive field of the larger K's that allows that. It's worth mentioning that local features are important for prediction as we get good results for lower K's but limiting the model class capicity exclusively to local features has reduces accuracy, as we witness in these graphs. In terms of train accuracy we can see that it is not always the case where K=128 generates the best results. We can learn from this that the K=128 generalizes much better and this is again, a result of global feature enablement, which are harder to learn and generally give more insightfull correlation about the input. This can also be seen by the test loss that increased the least towards the end. It's worth mentioning that for L=8 and K=128 we got perfect accuracy towards the end, and higher test loss which does indicate overfit, but we did get the best results using this model in terms of test accuracy across all graphs in this experiment.

As for the results we got in comparisent to experiment 1.1, this experiment also shows us that deeper network generate better prediction as we observed in this experiment. We also see that in general the K=128 had better results, this was not tested for in the previous experiment which tells about the importance of the kernel size for prediction. The two experiments together show the close relation between the two parameters, simply put, the filters size is more about the spatial components of the model and the depth is more about the model's ability to learn higher level represntation which generate higher level features. But in practice, they both have to do with both aspects of the model. The deeper we go in the network the receptive field increases which dictates what spatial features we can learn (we wont be able to learn features that require information outside the receptive field). The broader the filter size, the more high level global features we can learn (features that require a very large receptive field). It is crucial to know the implication of choosing one parameter on the other as they both direct and indirect effect on the model's preformance. 
"""

part5_q3 = r"""
**Your answer:**

All models got high accuracy both in test and train. Having said that, it is apparent that all models overfitted a bit as the train loss monotonously decreases while the test loss starts to increase at a certain point. This makes sense because these networks are quite shallow so it is easy to overfit. In addition, the model class capicity is much smaller and so approximation error could be high making generalization worse, this explains the diffrent trends between train loss and test loss towards the end. In comparisent to previous experiments we see that the models did a better job in test accuracy as in some case the models passed the 75% mark. We understand that varying filter size might improve results and also increasing it gradually gives a nice growth of reception field along the layers, which is very important in the networks architecture.
"""

part5_q4 = r"""
**Your answer:**
In general, we see that the Residual network preoforms better than the CNN networks we tested. This makes sense because in this model we added batchnorm and dropout which increase generaliztion. We have also added residual blocks that allow to better optimize on the given dataset and use an even deeper network, thus increasing model class capicity. We also see here that deeper networks tend to preform better, in the K=32 case it is more apparent while in the K=[64,128,256] we see that L=4 and L=8 did quite similar but significantly better than L=2. We also notice that K=[64,128,256] graphs were better than K=32 and we actually got the best test accuracy across all experiments using these filter sizes. It can also be seen how the L=32, K=32 generalizes very nicely as training accuracy and test accuracy are very similar. It's test accuracy is a bit lower compared to other K=[64,128,256] but that might be due to higher approximation error as it's model class capcity is smaller. All of these points add up nicely with previous experiments as we see that deeper networls work better, larger filters work better- and the two parameters relation is also a crucial element of the networks architrcture.

When we compare to experiment 1.1 we see the ResNet did a better job due the reasons we stated- better generalization by using more sophisticated regularization methods and also better approximation as ResNet allows for much deeper networks to be learned due the residual blocks.

When we compare to experiment 1.3 we see that in this experiment we got better results because we allowed for deeper networks and also larger filters, the combination of both, similarly to what we explained in the last paragraph, generated much better predictions.
"""
